 ...
 */
		...
		ajax : function(data, callback, settings) {//ajax配置为function,手动调用异步查询
			...
			$.ajax({
		            ...
		            success: function(result) {
						...
						for(var i=0;i<member.areaGroup.length;i++){
							$("#areaName option").each(function () {
								 var val = $(this).val(); //获取单个value
								 arr.push(val);
							 });
							var obj =member.areaGroup[i];
								 if($.inArray(obj, arr)==-1){
								 $("<option value='"+obj+"'>"+obj+"</option>").appendTo("#areaName");
							 }
						};
	...
	$("#userAdd").click(function(){
		window.location.href="Super_User_add.html";
	});
...
//		_table.draw(false);
	...
	$('#dropdown-export').on('click', function() {
		...
		window.location.href="../admin/User_exportUsers"+"?searchText="+searchText +"&startTime="+startTime+"&endTime="+endTime+"&areaName="+areaName;
...
var userManage = {
	...
	getQueryCondition : function(data) {
		...
		if (userManage.fuzzySearch) {
			...
			param.areaName = obj.options[index].value;
		}
