	...
	$.extend(true,{},CONSTANT.DATA_TABLES.DEFAULT_OPTION, {
        ...
        columns: [
            ...
            },
            {
				...
				var oadata = userManage.FormatHtml(data)
				if(oadata.length>20){
						var odata = oadata.substr(0, 20);
						return '<span title="' + oadata + '">' + odata + '...'+'</span>';
					} else if(oadata.indexOf('<img src=')>=0){
						return '<span title="' + data + '">' + "图片" + '</span>';
					}else
                	return '<span title="' + oadata + '">' + oadata + '</span>';
				}
...
//		_table.draw(false);
	...
	$table.on("change",":checkbox",function() {
		...
		userManage.currentItem = item;
		window.location.href="Notice_edit.html?noticeId="+item.id;
	})
	...
	deleteItem : function(selectedItems) {
		...
		if (selectedItems&&selectedItems.length) {
			if (selectedItems.length == 1) {
				...
				$.post("../admin/Notice_delete",{noticeId:ids}); 
